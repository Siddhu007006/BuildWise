...
import { SavedProjectsSection, saveProjectLocally, getLocalProject } from '@/components/dashboard/saved-projects';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // Check if there's freshly generated plan data from the planner page
    const stored = localStorage.getItem('planResults');
    if (stored) {
      try {
        const parsed = JSON.parse(stored);
        // Save it as a project card in localStorage
        saveProjectLocally({
          plot_area: parsed.projectDetails?.builtUpArea || 0,
          floors: parsed.projectDetails?.numberOfFloors || 1,
          timeline_months: parsed.projectDetails?.estimatedTimeline || 0,
          total_cost: parsed.costBreakdown?.totalCost || 0,
          plan_data: parsed,
        });
        // Trigger re-render of saved projects list
        setRefreshKey(prev => prev + 1);
      } catch (e) {
        console.error('Failed to parse planResults:', e);
      }
  ...
  const handleSelectProject = async (projectId: string | number) => {
    ...
    if (localProject?.plan_data) {
      setData(localProject.plan_data);
    } else {
      setData(null);
    }
    setLoadingDetail(false);
    ...
    <>
      ...
      <main className="min-h-screen bg-background pt-24 pb-20">
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          ...
          <SavedProjectsSection key={refreshKey} onSelectProject={handleSelectProject} />
        </div>
      </main>
    </>
  );
